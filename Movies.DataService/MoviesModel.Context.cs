//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace Movies.DataService
{
    using System;
    using System.Data.Entity;
    using System.Data.Entity.Infrastructure;
    
    public partial class ComcastEntities : DbContext
    {
        public ComcastEntities()
            : base("name=ComcastEntities")
        {
        }
    
        protected override void OnModelCreating(DbModelBuilder modelBuilder)
        {
            throw new UnintentionalCodeFirstException();
        }
    
        public virtual DbSet<CUSTOMERTABLE> CUSTOMERTABLEs { get; set; }
        public virtual DbSet<GENRESTABLE> GENRESTABLEs { get; set; }
        public virtual DbSet<MOVIEGENRESTABLE> MOVIEGENRESTABLEs { get; set; }
        public virtual DbSet<MOVIESTABLE> MOVIESTABLEs { get; set; }
        public virtual DbSet<RATINGTABLE> RATINGTABLEs { get; set; }
    }
}
